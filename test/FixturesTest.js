...
describe("FixturesTest", function () {
    if (require('../hardhat.config.js').networks.hardhat.forking.enabled) {
        it("Mainnet fork setup", async function () {
            const {
            ...
            expect(await uniswapFactory.owner()).to.not.equal(localSigner);
            expect(await ethers.provider.getCode(descriptorLibrary.target)).to.not.equal("0x");
            ...
            expect(await tickLens.getPopulatedTicksInWord("0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640", 0n)).to.not.eql([]);
            expect(await ethers.provider.getCode(multicall.target)).to.not.equal("0x");
            expect(await ethers.provider.getCode(multicall2.target)).to.not.equal("0x");
            expect(await ethers.provider.getCode(permit2.target)).to.not.equal("0x");
