...
describe("FixturesTest", function () {
    it("Mainnet fork setup [forking]", async function () {
        ...
        if (nonfungiblePositionManager.target != zeroAddress) expect(await nonfungiblePositionManager.WETH9()).to.equal(weth.target);
        ...
        if (universalRouter.target != zeroAddress) expect(await universalRouter.V3_POSITION_MANAGER()).to.equal(nonfungiblePositionManager.target);
    ...
    it("Default fixture [local]", async function () {
        ...
        const {
            weth, tokenDescriptor, nonfungiblePositionManager, swapRouter01, swapRouter02, quoter01, quoter02, universalRouter
        } = await loadFixture(UniswapV3DeploymentFixture);

        expect(await tokenDescriptor.WETH9()).to.equal(weth.target);
        expect(await nonfungiblePositionManager.WETH9()).to.equal(weth.target);
        ...
        expect(await universalRouter.V3_POSITION_MANAGER()).to.equal(nonfungiblePositionManager.target);
